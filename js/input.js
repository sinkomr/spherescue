...
class Input {
  ...
  initTouch(canvas) {
    // Swipe = move (one cell per ~40px), tap = drop, two-finger tap / long-press = grab toggle.
    let sx = 0, sy = 0, moved = false, id = null, lastX = 0, lastY = 0, t0 = 0;
    const opt = { passive: false };
    canvas.addEventListener('touchstart', (e) => {
      e.preventDefault();
      this.anyKey = true;
      if (e.touches.length === 2) { this.queue.push({ type: 'grab' }); this.held.grab = !this.held.grab; return; }
      const t = e.changedTouches[0];
      id = t.identifier; sx = lastX = t.clientX; sy = lastY = t.clientY;
    ...
    canvas.addEventListener('touchmove', (e) => {
      ...
      if (!t) return;
      const STEP = 38;
      while (t.clientX - lastX > STEP) { this.queue.push({ type: 'dir', dir: 'right' }); lastX += STEP; moved = true; }
      while (lastX - t.clientX > STEP) { this.queue.push({ type: 'dir', dir: 'left' }); lastX -= STEP; moved = true; }
      while (t.clientY - lastY > STEP) { this.queue.push({ type: 'dir', dir: 'down' }); lastY += STEP; moved = true; }
      while (lastY - t.clientY > STEP) { this.queue.push({ type: 'dir', dir: 'up' }); lastY -= STEP; moved = true; }
    }, opt);
    canvas.addEventListener('touchend', (e) => {
      ...
      if (!moved && dt < 260) this.queue.push({ type: 'drop' });
      id = null;
    }, opt);
  }
